...
虽然上面描述了ReplicaSet与ReplicationController，但是一般情况下，管理员不会直接创建ReplicaSet与ReplicationController，而是通过创建Deployment这一个控制器来实现对Pod的创建和管理。本质上，创建Deployment后，Deployment控制器将立刻创建一个ReplicaSet副本集，并由ReplicaSet创建所需要的Pod，其目的就是更好地实现版本的更新与回滚。
